...
public class MemberDao implements IMemberDao {
	...
	public MemberDao() {
		/*
		 * try { Context context = new InitialContext();//was와 연결된 context dataSource
		 * =(DataSource)context.lookup("java:comp/env/jdbc/Oracle");//oracle드라이버를 찾음.
		 * 
	...
	public int joinNickNameCheck(String nickName){//아디디 중복체크검사
		System.out.println("닉네임 : "+nickName);
		int cnt = -1;
		String query ="select count(name) from member where name='"+nickName+"'";
		String sid = this.template.queryForObject(query, String.class);
		...
		System.out.println("닉네임 중복체크결과 : "+cnt);
		return cnt;
	}//joinIdCheck 메서드닫음
